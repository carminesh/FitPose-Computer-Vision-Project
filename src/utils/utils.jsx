...
export function normalizeLandmarks(landmarks, frameHeight) {
    ...
    const shoulderDistance = Math.sqrt(Math.pow(leftShoulder.x - rightShoulder.x, 2) + Math.pow(leftShoulder.y - rightShoulder.y, 2)) * frameHeight;
...
export function detectWebcamMovement(currentLandmarks) {
    ...
    const averageMovement =
        currentLandmarks.reduce((sum, current, index) => {
